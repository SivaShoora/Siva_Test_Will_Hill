...
public class CustomAssertions {
	...
	public static void checkReturnValue(WebDriver driver) throws Exception{	//This method asserts return value
		Assert.assertEquals("To return:",AutomationHomePage.totalReturnLabel.getText(),"Assertion for Total return price label failed");
		Log.info("Asserted ToReturn Label" );
		Assert.assertEquals(LoginAndSubmitBet.expRetPrice,AutomationHomePage.totalReturnValue.getText(),"Assertion for total return price value failed");
	...
	public static void checkStackValue(WebDriver driver, String betValue) throws Exception{	//This method asserts stack value
		Assert.assertEquals("Total stake:",AutomationHomePage.totalStakeLabel.getText(), "Assertion for Total stake label failed");
		Log.info("Asserted TotalStake Label" );
		Assert.assertEquals(betValue,AutomationHomePage.totalStakeValue.getText(),"Assertion for total stake value failed");
		Log.info("Asserted TotalStake Value" );
	}
	
	public static void checkBalance(WebDriver driver, String betAmount) throws Exception{
		double balanceAfterBet = Double.parseDouble(AutomationHomePage.balanceLink.getText().substring(1));
		Assert.assertEquals(balanceAfterBet+Double.parseDouble(betAmount), LoginAndSubmitBet.balanceBeforeBet,0.001,"Balance after bet is incorrect");
